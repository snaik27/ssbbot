...
def charactername(comment,function,no_info,signature,replied_to):
        character= pd.read_csv('%s.csv'%function[1].capitalize(), encoding='utf-8')
        if (function[1].capitalize() == 'Cloud'): 
                comment.reply('#OP' +'\n  ------------ \n' +signature)
        elif (function[1].capitalize() == 'Bayonetta'):
                comment.reply('#(╯ಠ_ಠ）╯︵ ┻━┻' + '\n  ------------ \n' +signature)
        else:
                ...
                if (function[2]=='properties'):
                        properties = character.iloc[:,:4].copy()
                        properties.dropna(inplace=True)
                        pretty = tabulate(properties, headers='keys',tablefmt="pipe")
                        comment.reply('Smash 4 ' + fuction[1] + ' ' + function[2]+': \n' + pretty + '\n\n' + kurogane + '\n  ------------ \n' + signature)
                        
                elif(function[2]=='ground_moves'):
                        ground_moves=character.iloc[:,4:10].copy()
                        ground_moves.dropna(inplace=True)
                        pretty =tabulate(ground_moves,headers='keys',tablefmt="pipe")
                        comment.reply('Smash 4 ' + fuction[1] + ' ' + function[2]+': \n' + pretty + '\n\n' + kurogane + '\n  ------------ \n' + signature)
                        
                elif(function[2]=='aerials'):
                        aerials = character.iloc[:,11:20].copy()
                        aerials.dropna(inplace=True)
                        pretty =tabulate(aerials,headers='keys',tablefmt="pipe")
                        comment.reply('Smash 4 ' + fuction[1] + ' ' + function[2]+': \n' + pretty + '\n\n' + kurogane + '\n  ------------ \n' + signature)
                        
                elif(function[2]=='specials'):
                        specials=character.iloc[:,21:27].copy()
                        specials.dropna(inplace=True)
                        pretty = tabulate(specials,headers='keys',tablefmt="pipe")
                        comment.reply('Smash 4 ' + fuction[1] + ' ' + function[2]+': \n' + pretty + '\n\n' + kurogane + '\n  ------------ \n' + signature)
                else:
...
def define(comment,function,no_info,signature,replied_to, terms):

    #Format user inputs to work with our data
    function=function[2:]
    for word in range(0,len(function)):
        function[word]=re.sub('[\W\s\_]','',function[word])
        function[word]=re.sub('(ing|ly|ed|ious|ies|ive|es|s|ment)?$','',function[word])
    function= ''.join(function)
    function=function.lower()


    if function in terms:
        term_url = requests.get(terms[function])
        soup = BeautifulSoup(term_url.text,'lxml')
